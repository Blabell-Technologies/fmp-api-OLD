...
module.exports = async (req, res, next) => {
  ...
  catch (error) { return res.status(400).json({ code: 400, type: 'invalid-request-error' }) };
  ...
  //#region Validaciónes de datos
    ...
    //#region Verificamos que la fecha sea valida
      if (!/(0?[1-9]|1[0-2])[^\w\d\r\n:](0?[1-9]|[12]\d|30|31)[^\w\d\r\n:](\d{4}|\d{2})\ (0?([01]?\d|2[0-3]):([0-5]?\d))/gm.test(disappearance_date)) return res.status(406).json({ code: 406, type: 'validation-error', field: 'disappearance_date' });
